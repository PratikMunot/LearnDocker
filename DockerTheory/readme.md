...
### Docker Image
> Docker images are the read-only binary templates used to create Docker Containers (ie. an environment of your own choice). In other words its a blueprints of our application which form the basis of containers. A docker image is described in text file called a Dockerfile, which has a simple, well-defined syntax. An image does not have states and never changes. Docker Engine provides the core Docker technology that enables images and containers.
To build your own image, you create a Dockerfile with a simple syntax for defining the steps needed to create the image and run it. Each instruction in a Dockerfile creates a layer in the image. When you change the Dockerfile and rebuild the image, only those layers which have changed are rebuilt. This is part of what makes images so lightweight, small, and fast, when compared to other virtualization technologies.
An image can also be considered a snapshot of your current working environment. It uses a private container registry to share container images within the enterprise and also uses public container registry to share container images within the whole world. Metadata is also used by docker images to describe the container's abilities.
...
### Docker Containers
> A Container is created from Docker images and run the actual application.  Containers are the structural units of Docker, which is used to hold the entire package that is needed to run the application. 
...
You can use Command Line Interface (CLI) commands to run, start, stop, move, display all/running containers or delete a container. 
...
allocate any RAM and disk space for the applications and It automatically generates storage and space according to the application requirement. 
